...
4. 在 v1.11.0 之后，`keadm init` 和 `keadm beta init` 默认实现容器化部署CloudCore，如果您希望以二进制包来部署CloudCore，请参考 [`keadm deprecated init`](#keadm-deprecated-init).
...
> 在 v1.11.0 版本之后，`keadm beta init` 将直接使用容器化方式部署云端组件 cloudcore。
...
**重要提示：**
在 `keadm init` 和 `keadm beta init`中：
1. 自定义 `--set key=value`
...
### keadm manifest generate 和 keadm beta manifest generate

`keadm manifest generate` 和 `keadm beta manifest generate` 可以帮助我们快速渲染生成期望的 manifests 文件，并输出在终端显示。
