...
5. Also, check that the `podSandboxImage` is correctly set as defined in [Modification in edgecore.yaml](../configuration/config#modification-in-edgecoreyaml).
...
You can also enable `kubectl logs` feature refer to this [guide](../troubleshooting/debug.md).