...
If you encounter problems during installation, such as cloudcore/edgecore don't start successfully, or they both start but edge nodes are always in `NotReady`, or pod cannot be deployed to edge nodes as what is described in [Deploy demo on edge nodes](../../user-guide/getting-started/install-with-binary.md#Deploy-demo-on-edge-nodes). This doc can help you debug how to fix the issues.
